...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const tools = items.map((item) => {
      ...
      const toolkit = typeof toolkitValue === "string"
        ? toolkitValue
        : toolkitValue && typeof toolkitValue === "object" && typeof (toolkitValue as Record<string, unknown>).slug === "string"
          ? String((toolkitValue as Record<string, unknown>).slug)
          : "";
      const slug = typeof value.slug === "string" ? value.slug : typeof value.name === "string" ? value.name : "";
      return {
        slug,
