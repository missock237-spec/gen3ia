...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const connectors = await listDeveloperProjectConnectors(guard.context.userId, id);
      ...
      connectors.map(async (item) => {
        ...
        if (status !== item.status) {
          return upsertDeveloperProjectConnector({
            ...
            await adminDb.collection("developerProjectConnectors").doc(item.id).update({
              status,
              updatedAt: Date.now(),
            });
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to sync connectors" },
      { status: 403 },
    );
  }
}
