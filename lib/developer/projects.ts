...
export async function listDeveloperProjects(ownerId: string) {
  await assertDeveloperRole(ownerId);
...
export async function createDeveloperProject(
  ownerId: string,
  input: { name: string; description?: string; framework?: string },
) {
  await assertDeveloperRole(ownerId);
...
export async function updateDeveloperProject(
  ...
  input: Partial<
    Pick<
      DeveloperProject,
      "name" | "description" | "framework" | "environment" | "status"
    >
  >,
) {
  await assertDeveloperRole(ownerId);
  ...
  if (typeof input.framework === "string") {
    patch.framework = input.framework.trim().slice(0, 40);
  }
  if (
    input.environment &&
    ["development", "preview", "production"].includes(input.environment)
  ) {
