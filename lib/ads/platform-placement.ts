...
export function validatePlatformAdInput(input: PlatformAdInput): PlatformAdInput {
  ...
  if (normalized.format === "image" && !normalized.imageUrl) {
    throw new Error("Une publicité image doit définir imageUrl.");
  }
  if (normalized.format === "video" && !normalized.videoUrl) {
    throw new Error("Une publicité vidéo doit définir videoUrl.");
  }
  if (normalized.startsAtMs && normalized.endsAtMs && normalized.endsAtMs <= normalized.startsAtMs) {
...
export async function listPlatformAds(placement?: string): Promise<PlatformAd[]> {
  let query = adminDb.collection(COLLECTION).limit(200);
  if (placement) query = query.where("placement", "==", placement).limit(200) as typeof query;
...
function isCurrentlyEligible(ad: PlatformAd, nowMs: number): boolean {
  return ad.enabled
    && (!ad.startsAtMs || ad.startsAtMs <= nowMs)
    && (!ad.endsAtMs || ad.endsAtMs > nowMs);
}
...
export async function choosePlatformAd(placement: string): Promise<PlatformAd> {
  const now = Date.now();
  const eligible = (await listPlatformAds(placement)).filter((ad) => isCurrentlyEligible(ad, now));
...
export async function updatePlatformAd(id: string, patch: Partial<PlatformAdInput>): Promise<PlatformAd> {
  ...
  await existing.ref.update({ ...merged, updatedAtMs });
...
function serializeAd(id: string, data: FirebaseFirestore.DocumentData): PlatformAd {
  return {
    id,
