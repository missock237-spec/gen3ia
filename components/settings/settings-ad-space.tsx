...
export function SettingsAdSpace() {
  ...
  if (loading) {
    return <section className="rounded-3xl border border-neutral-200 bg-white p-5"><div className="h-32 animate-pulse rounded-2xl bg-neutral-100" /></section>;
  }
  ...
  return (
    <section className="overflow-hidden rounded-3xl border border-neutral-200 bg-white shadow-[0_8px_30px_-18px_rgba(15,23,42,0.2)]" aria-label="Publicité">
      <div className="flex items-center justify-between gap-3 border-b border-neutral-100 px-5 py-3">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[.2em] text-neutral-400">Espace publicitaire</p>
          <p className="mt-0.5 text-[10px] text-neutral-400">Publicité sponsorisée</p>
        </div>
        <span className="rounded-full bg-neutral-100 px-2.5 py-1 text-[10px] font-semibold text-neutral-500">{ad.advertiser}</span>
      </div>
      ...
      <div className="space-y-3 p-5">
        <div>
          <h2 className="text-lg font-semibold text-neutral-900">{ad.title}</h2>
          {ad.description && <p className="mt-1.5 text-sm leading-6 text-neutral-500">{ad.description}</p>}
          {ad.text && <p className="mt-2 text-sm font-medium text-neutral-700">{ad.text}</p>}
        </div>
        <a
          href={ad.targetUrl}
          target="_blank"
          rel="noopener noreferrer sponsored"
          onClick={() => void track("click")}
          className="inline-flex items-center rounded-full bg-neutral-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-neutral-700"
        >
          {cta}
        </a>
      </div>
