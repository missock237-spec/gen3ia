...
export function VoiceAgentSetup({ agentId, onDone }: Props) {
  ...
  const [provider, setProvider] = useState<"twilio" | "plivo">("twilio");
  ...
  const search = async () => {
    ...
    try {
      const query = "/api/voice/numbers?provider=" + provider + "&country=" + encodeURIComponent(country) + (areaCode.trim() ? "&areaCode=" + encodeURIComponent(areaCode.trim()) : "");
  ...
  const buy = async (phoneNumber: string) => {
    ...
    try {
      const response = await authFetch("/api/voice/numbers", {
        ...
        body: JSON.stringify({ agentId, phoneNumber, provider }),
      });
      const data = await response.json();
    ...
    <section className="g3-card anim-slide-up p-5 md:p-7">
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-2">
        <div className="space-y-4">
          <div className="rounded-2xl border border-[var(--g3-border)] bg-neutral-50 p-4">
...
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (value: boolean) => void }) {
  return <label className="flex cursor-pointer items-center justify-between rounded-xl border border-[var(--g3-border)] bg-neutral-50 px-3 py-2.5 text-sm"><span>{label}</span><button type="button" role="switch" aria-checked={checked} data-on={checked} className="g3-switch" onClick={() => onChange(!checked)} /></label>;
}
